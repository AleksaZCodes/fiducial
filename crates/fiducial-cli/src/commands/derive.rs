...
use crate::{
    config::{Config, DeployShape, SqlDialect, CONFIG_FILE},
...
fn run_fid_deploy(pipeline: &Pipeline, working_dir: &Path) -> Result<()> {
    ...
    if vendor != "cloudflare" {
        bail!(
            "fid-deploy: [adapters] deploy = \"{vendor}\" — this executor \
             generates a Cloudflare Worker config. Set deploy = \"cloudflare\", \
    ...
    let plan = deploy_plan(&config)?;

    // The **output path decides the format**, the same way `pipelines/brand.toml`
    // lets a file name say which artifact it is. `wrangler` reads either, and
    // which one a product wants is not a second decision: an `apps/worker/`
    // Worker conventionally carries `wrangler.toml`, while a SvelteKit product
    // carries `wrangler.jsonc` at its root because that is the file
    // `@sveltejs/adapter-cloudflare` looks for.
    let out = if target.ends_with(".json") || target.ends_with(".jsonc") {
        render_wrangler_jsonc(&plan)
    } else {
        render_wrangler_toml(&plan)
    };
...
/// Everything a `wrangler` config states, derived once.
///
/// Computed apart from rendering because there are now two output formats and
/// one set of facts. The alternative — a second function emitting JSONC from
/// the same declarations — is a second derivation of the bindings, and the two
/// would disagree the first time a contract was added to one of them.
struct DeployPlan {
    name: String,
    main: String,
    ...
    route: String,
    /// `(directory, binding)` for a SvelteKit product's static assets.
    assets: Option<(String, String)>,
    observability: bool,
    vars: Vec<(String, String)>,
    d1: Option<(String, String)>,
    r2: Option<String>,
    queue: Option<String>,
...
fn deploy_plan(config: &Config) -> Result<DeployPlan> {
    let product = &config.product.name;
    let d = &config.deploy;
    let shape = d.resolve_shape()?;

    // The Worker's name is its identity on the account: change it and you have
    // not renamed a deployment, you have created a second one and orphaned the
    // first, along with whatever traffic and bindings pointed at it. So a
    // product that has ever deployed declares it, and the default only applies
    // to one that has not.
    let name = if d.name.is_empty() {
        match shape {
            // A SvelteKit product's Worker *is* the product, so `-worker`
            // would be noise in the one place a name is load-bearing.
            DeployShape::SvelteKit => product.clone(),
            DeployShape::Worker => format!("{product}-worker"),
        }
    } else {
        d.name.clone()
    };

    // For a SvelteKit product the entrypoint is **build output**, not a source
    // file: the adapter writes it, and `src/index.ts` — the standalone
    // Worker's default — names a file that does not exist.
    let main = if d.main.is_empty() {
        match shape {
            DeployShape::SvelteKit => ".svelte-kit/cloudflare/_worker.js".to_string(),
            DeployShape::Worker => "src/index.ts".to_string(),
        }
    } else {
        d.main.clone()
    };

    // A standalone Worker serves what its code returns. A SvelteKit one also
    // serves the adapter's static output, through a binding — omit it and
    // every asset 404s while the Worker itself looks healthy.
    let assets = match shape {
        DeployShape::SvelteKit => Some((
            d.resolved_assets_directory().to_string(),
            d.resolved_assets_binding().to_string(),
        )),
    ...
    // ── Derived bindings ────────────────────────────────────────────────
    // Each appears exactly when [adapters] selected the vendor that reads it,
    // under the binding name that vendor's adapter class reads.
    let d1 = if config.adapters.get("database") == Some("d1") {
        let db_name = if d.d1_database_name.is_empty() {
            product.clone()
        } else {
            d.d1_database_name.clone()
        };
        Some((db_name, d.d1_database_id.clone()))
    } else {
        None
    };
    let r2 = if config.adapters.get("storage") == Some("r2") {
        Some(d.r2_bucket_name.clone())
    } else {
        None
    };
    let queue = if config.adapters.get("queue") == Some("cloudflare-queues") {
        Some(d.queue_name.clone())
    } else {
        None
    };

    // KV is declared, not derived: a product's own namespace is a fact about
    // its account rather than a vendor selection, and there is no contract to
    // infer it from.
    let kv: Vec<(String, String)> = d
        .kv_namespaces
        .iter()
        .map(|n| (n.binding.clone(), n.id.clone()))
        .collect();
    ...
    // `newsletter = "resend"`, so this collects and deduplicates (preserving
    // first-occurrence order) — otherwise a product selecting both emits it
    // twice.
    ...
    Ok(DeployPlan {
        name,
        main,
        compatibility_date: d.compatibility_date.clone(),
        compatibility_flags: d.compatibility_flags.clone(),
        route: d.route.clone(),
        assets,
...
const DEPLOY_HEADER: [&str; 5] = [
    "generated by `fid derive` — do not edit",
    "source of truth: [deploy] and [adapters] in fiducial.toml",
    "",
    "Bindings are DERIVED from [adapters]: selecting a vendor there is what",
    "puts its binding here, under the fixed name the adapter reads.",
];

fn render_wrangler_toml(p: &DeployPlan) -> String {
    let mut out = String::new();
    for line in DEPLOY_HEADER {
        out.push_str(&format!(
            "#{}{line}\n",
    ...
    out.push_str(&format!(
        "compatibility_date = \"{}\"\n",
        p.compatibility_date
    ));
    if !p.compatibility_flags.is_empty() {
    ...
    if let Some((directory, binding)) = &p.assets {
        out.push_str("\n# The adapter's build output, served through a binding.\n");
        out.push_str("[assets]\n");
    ...
    if let Some((db_name, db_id)) = &p.d1 {
        ...
        out.push_str(&format!("database_id = \"{db_id}\"\n"));
    }
    if let Some(bucket) = &p.r2 {
        out.push_str("\n# [adapters] storage = \"r2\" → R2Storage reads env.BUCKET\n");
        out.push_str("[[r2_buckets]]\n");
        out.push_str("binding = \"BUCKET\"\n");
        out.push_str(&format!("bucket_name = \"{bucket}\"\n"));
    }
    ...
    for (binding, id) in &p.kv {
        out.push_str("\n# [deploy.kv_namespaces] — this product's own namespace.\n");
...
/// The same facts as JSONC, for a product whose config is `wrangler.jsonc`.
///
/// JSON with comments rather than plain JSON, because the comments are load
/// bearing here: this file is generated, and a reader who does not know that
/// will hand-edit it and lose the edit on the next `fid derive`. `wrangler`
/// accepts JSONC, and so does `@sveltejs/adapter-cloudflare`.
fn render_wrangler_jsonc(p: &DeployPlan) -> String {
    ...
    fields.push(format!("  \"name\": {}", q(&p.name)));
    fields.push(format!("  \"main\": {}", q(&p.main)));
    fields.push(format!(
        "  \"compatibility_date\": {}",
        q(&p.compatibility_date)
    ));
    if !p.compatibility_flags.is_empty() {
        let flags: Vec<String> = p.compatibility_flags.iter().map(|f| q(f)).collect();
        fields.push(format!("  \"compatibility_flags\": [{}]", flags.join(", ")));
    }
    if !p.route.is_empty() {
        fields.push(format!("  \"route\": {}", q(&p.route)));
    }
    if p.observability {
        fields.push("  \"observability\": { \"enabled\": true }".to_string());
    }
    if let Some((directory, binding)) = &p.assets {
        fields.push(format!(
    ...
    let vars: Vec<String> = p
        ...
        .collect();
    fields.push(format!("  \"vars\": {{ {} }}", vars.join(", ")));
    ...
    if let Some(queue) = &p.queue {
        fields.push(format!(
            "  // [adapters] queue = \"cloudflare-queues\" → CloudflareQueue reads env.QUEUE\n  \"queues\": {{ \"producers\": [{{ \"binding\": \"QUEUE\", \"queue\": {} }}] }}",
            q(queue)
        ));
    }
    if !p.kv.is_empty() {
        let entries: Vec<String> =
            p.kv.iter()
                .map(|(binding, id)| {
                    format!("    {{ \"binding\": {}, \"id\": {} }}", q(binding), q(id))
                })
                .collect();
        fields.push(format!(
            "  // [deploy.kv_namespaces] — this product's own namespaces. The id is\n  // load-bearing: point a deploy at a different one and the data is\n  // silently gone rather than missing.\n  \"kv_namespaces\": [\n{}\n  ]",
            entries.join(",\n")
        ));
    }
    ...
    if !p.secrets.is_empty() {
        ...
        out.push_str("  // Never written here — a generated file in the repository is\n");
        out.push_str("  // the one place a secret's value must not be.\n");
    }
