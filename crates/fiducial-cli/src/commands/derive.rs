...
///
/// The schema is a **derivation of the types**, which is what keeps the two in
/// step: the `CHECK` lists are exactly the variants `Principal`, `Resource`
/// and `Role` define, so adding a role and forgetting the migration is not a
/// thing that can happen quietly — `fid derive --check` fails instead.
///
/// SQLite dialect, because D1 is SQLite and D1 is the one real `database`
/// vendor. A Postgres vendor would add a dialect here rather than a second
/// hand-written file.
fn run_fid_identity(pipeline: &Pipeline, working_dir: &Path) -> Result<()> {
    ...
    let content = format!(
        "-- generated by `fid derive` — do not edit\n\
         -- source of truth: the identity model in `fiducial-identity`,\n\
         -- plus [identity] in fiducial.toml for the table name.\n\
         --\n\
         -- A grant is one row: who, over what, how much.\n\
         --\n\
         -- The CHECK lists below are the `Principal`, `Resource` and `Role`\n\
         -- variants themselves. `anonymous` is absent on purpose: `can()`\n\
         -- refuses an unidentified principal, so the database refuses to\n\
         -- store a row the rule would ignore.\n\
         \n\
         CREATE TABLE IF NOT EXISTS {table} (\n\
         \u{20} principal_kind TEXT NOT NULL CHECK (principal_kind IN ({principals})),\n\
         \u{20} principal_id   TEXT NOT NULL CHECK (principal_id GLOB '*[^0]*'),\n\
         \u{20} resource_kind  TEXT NOT NULL CHECK (resource_kind IN ({resources})),\n\
         \u{20} -- '' for a platform-scoped grant, which covers every resource.\n\
         \u{20} resource_id    TEXT NOT NULL,\n\
         \u{20} role           TEXT NOT NULL CHECK (role IN ({roles})),\n\
         \u{20} granted_at     TEXT NOT NULL,\n\
         \u{20} -- One role per principal per resource: granting again replaces.\n\
         \u{20} PRIMARY KEY (principal_kind, principal_id, resource_kind, resource_id)\n\
         );\n\
         \n\
         -- `grantsFor(principal)` — the read on the authorization path.\n\
         CREATE INDEX IF NOT EXISTS {table}_by_principal\n\
         \u{20} ON {table} (principal_kind, principal_id);\n\
         \n\
         -- `grantsOn(resource)` — the read a sharing UI makes.\n\
         CREATE INDEX IF NOT EXISTS {table}_by_resource\n\
         \u{20} ON {table} (resource_kind, resource_id);\n",
        table = table,
        principals = quoted(GRANT_PRINCIPAL_KINDS),
        resources = quoted(GRANT_RESOURCE_KINDS),
        roles = quoted(GRANT_ROLES),
    );

    let out = pipeline
        .outputs
        .first()
        .ok_or_else(|| anyhow::anyhow!("fid-identity: pipeline declares no outputs"))?;
    let abs = working_dir.join(out);
    if let Some(parent) = abs.parent() {
