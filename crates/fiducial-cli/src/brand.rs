...
/// The brand facts, as a TypeScript module the application can import.
///
/// ## Why this output exists
///
/// Every other output of this pipeline is an artifact a *crawler* reads:
/// `robots.txt`, `sitemap.xml`, the manifest, the JSON-LD. None of them is
/// importable by the application, so the moment application code needs the
/// domain — for `metadataBase`, a canonical URL, an absolute OG image, an
/// email link — there is nowhere to get it and it gets retyped.
///
/// It gets retyped into exactly one line, which is what makes it dangerous.
/// The duplicate is invisible in review, and it is discovered the day the
/// domain changes and the sitemap, the manifest and the legal pages all move
/// while the canonical URL and the OG tags quietly keep pointing at the old
/// host. Nothing fails. The product just advertises the wrong origin.
///
...
pub fn render_brand_ts(
    ...
    // exactly right, and the failure is silent — the module still compiles,
    // it is just indented like a quote block. Lines and a join cannot do that.
    let lines = [
        "// Generated by `fid derive` from [brand] in fiducial.toml. Do not edit.".to_string(),
        "//".to_string(),
        "// The domain lives in exactly one place and this is its derivation, not a".to_string(),
        "// second copy of it. Import from here rather than writing an origin into".to_string(),
        "// application code: the same value is already feeding robots.txt,".to_string(),
        "// sitemap.xml, site.webmanifest, organization.jsonld and every legal page,".to_string(),
        "// and a hand-typed origin is the one that will be missed when it changes.".to_string(),
        String::new(),
        format!("export const legalName = {legal_name:?} as const"),
        format!("export const tradingName = {trading_name:?} as const"),
        format!("export const shortName = {short_name:?} as const"),
        format!("export const domain = {domain:?} as const"),
        ...
        String::new(),
        "/** An absolute URL for a root-relative path. */".to_string(),
        "export const absoluteUrl = (path: string): string =>".to_string(),
        "  new URL(path, siteUrl).toString()".to_string(),
    ];
    format!("{}\n", lines.join("\n"))
}
