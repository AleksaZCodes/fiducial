...
export class WebSerialTransport implements Transport {
  ...
  private constructor(port: SerialPort, writer: WritableStreamDefaultWriter<Uint8Array>, baudRate: number) {
    this.port = port
    this.writer = writer
  ...
  static async open(options: SerialOptions = {}): Promise<WebSerialTransport> {
    const { baudRate = 115200, filters } = options
    const port = await navigator.serial.requestPort({ filters: filters ?? [] })
    await port.open({ baudRate })
    const writer = port.writable!.getWriter()
    return new WebSerialTransport(port, writer, baudRate)
  }
